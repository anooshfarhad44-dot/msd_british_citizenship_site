    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 min-w-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="relative shrink-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-gradient-to-br from-[#7a003c] to-[#7a003c] text-white font-black text-xs sm:text-sm flex items-center justify-center shadow-[0_4px_12px_rgba(27,111,168,0.25)] relative">
              {review.image ? (
                // 🔥 Agar image hai to Next.js Image component render hoga
                <Image 
                  src={review.image} 
                  alt={review.name} 
                  fill 
                ...
                // Fallback: Agar image null hai to initials dikhenge
                getInitials(review.name)
              )}
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#00b67a] border-2 border-white z-10" />
