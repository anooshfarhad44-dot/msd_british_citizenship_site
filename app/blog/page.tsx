        ...
        <div className="w-full max-w-[1120px] mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogPosts.map((post) => (
              ...
              >
                <div className="aspect-[16/9] bg-gradient-to-br from-[#7a003c] to-[#7a003c] flex items-center justify-center">
                  <span className="text-6xl">🇬🇧</span>
                </div>
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#7a003c] bg-[#e6f0f8] px-2.5 py-1 rounded-full">{post.category}</span>
                    <span className="text-[10px] text-[#4a6480]">{post.date}</span>
                  </div>
                  <h3 className="font-extrabold text-[#7a003c] text-lg mb-2 group-hover:text-[#7a003c] transition-colors leading-snug">
                    {post.title}
                  </h3>
                  <p className="text-[#4a6480] text-sm leading-relaxed line-clamp-3">{post.excerpt}</p>
                  <div className="mt-4 flex items-center gap-1 text-[#7a003c] font-black text-sm group-hover:gap-2 transition-all">
                    Read more <span>→</span>
                  </div>
                </div>
              </Link>
